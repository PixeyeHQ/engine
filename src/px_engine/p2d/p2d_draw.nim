...
proc pixel*(api: P2DrawAPI, x, y, z: f32; size: f32 = 1.px; color: Color = cwhite) =
  const indices = 0
  const vertices = 6
  r2d.draw(indices, vertices, R2D_GEOMETRY):
    r2d.color(color)
...
# @api circle
#------------------------------------------------------------------------------------------
proc circleSector*(api: P2DrawAPI, x, y, z: f32, radius: f32, angleBegin: f32, angleEnd: f32, segments: i32,
    color: Color = cwhite) =
  const indices = 0
...
proc circleSectorLine*(api: P2DrawAPI, x, y, z: f32; radius: f32, thickness: f32, segments: i32, angleBegin: f32,
    angleEnd: f32, color: Color = cwhite): tuple[v1, v2: Vec] {.discardable.} =
  const indices = 0
...
proc circleSectorLineScaled*(api: P2DrawAPI, x, y, z: f32; scaleX: f32, scaleY: f32, radius: f32, thickness: f32, segments: i32, angleBegin: f32,
    angleEnd: f32, color: Color = cwhite): tuple[v1, v2: Vec] {.discardable.} =
  const indices = 0
  let angleStep = (angleEnd-angleBegin)*DEG2RAD / f32(segments)
  var cosBegin = cos(angleBegin)
  ...
  r2d.draw(indices, vertices, R2D_GEOMETRY):
    ...
    for i in 0..<segments:
      let cosEnd = cosBegin * cosStep - sinBegin * sinStep
      let sinEnd = cosBegin * sinStep + sinBegin * cosStep
      let x1 = cx + radius1 * cosBegin * scaleX
      let y1 = cy + radius1 * sinBegin * scaleY
      let x2 = cx + radius1 * cosEnd * scaleX
      let y2 = cy + radius1 * sinEnd * scaleY
      let x3 = cx + radius2 * cosEnd * scaleX
      let y3 = cy + radius2 * sinEnd * scaleY
      let x4 = cx + radius2 * cosBegin * scaleX
      let y4 = cy + radius2 * sinBegin * scaleY
      r2d.vertex(x1, y1, z)
      r2d.vertex(x4, y4, z)
      r2d.vertex(x3, y3, z)
      r2d.vertex(x1, y1, z)
...
proc circleLineScaled*(api: P2DrawAPI, x, y, z: f32, scaleX: f32, scaleY: f32, radius: f32, thickness: f32, color: Color = cwhite) =
  const segments = 18
  api.circleSectorLineScaled(x, y, z, scaleX, scaleY, radius, thickness, segments, 0f, 360f, cWhite)
...
proc rectRound*(api: P2DrawAPI, x, y, z: f32, w, h: f32, cx, cy: f32, angle: f32, roundness: f32,
    color: Color = cwhite) =
  const indices = 0
  const segments = 8
  const vertices = 30 + 4 * segments * 3
...
proc rectRoundLine*(api: P2DrawAPI, x, y, z: f32; w, h: f32, thickness: f32, roundness: f32, color: Color = cwhite) =
  ...
  let angleStep = 90f / segments.float
  let angles = [180.0, 90.0, 0, 270]
  let centers = [vec(x+inRadius, y+inRadius), vec(x+w-inRadius, y+inRadius), vec(x+w-inRadius, y+h-inRadius), vec(
      x+inRadius, y+h-inRadius)]
  r2d.draw(indices, vertices, R2D_GEOMETRY):
    r2d.color(color)
    for indexCorner in 0..<4:
...
proc rectRoundLine*(api: P2DrawAPI, x, y, z: f32; w, h: f32, cx, cy: f32, angle: f32, thickness: f32, roundness: f32,
    color: Color = cwhite) =
  #[
  Centers: p16-p19
  |        P5 ================== P4
  ...
  r2d.draw(indices, vertices, R2D_GEOMETRY):
    ...
    for indexCorner in 0..<4:
      ...
      for index in 0..<segments:
        ...
        r2d.vertexr(c.x + sin(DEG2RAD*(a+angleStep)) * inRadius, c.y + cos(DEG2RAD*(a+angleStep))*inRadius, z, ox, oy,
            cos, sin)
        r2d.vertexr(c.x + sin(DEG2RAD*a) * outRadius, c.y + cos(DEG2RAD*a)*outRadius, z, ox, oy, cos, sin)
        r2d.vertexr(c.x + sin(DEG2RAD*(a+angleStep)) * inRadius, c.y + cos(DEG2RAD*(a+angleStep))*inRadius, z, ox, oy,
            cos, sin)
        r2d.vertexr(c.x + sin(DEG2RAD*(a+angleStep)) * outRadius, c.y + cos(DEG2RAD*(a+angleStep))*outRadius, z, ox, oy,
            cos, sin)
        r2d.vertexr(c.x + sin(DEG2RAD*a) * outRadius, c.y + cos(DEG2RAD*a)*outRadius, z, ox, oy, cos, sin)
        a += angleStep
    # Bottom Left Right
...
proc rectLine*(api: P2DrawAPI, x, y, z: f32; w, h: f32, cx, cy: f32, angle: f32, thickness: f32,
    color: Color = cwhite) =
  const indices = 0
  const vertices = 24
  let x = x - cx * w
...
proc rectRoundLine*(api: P2DrawAPI, x, y: f32; w, h: f32, cx, cy: f32, angle: f32, thickness: f32, roundness: f32,
    color: Color = cwhite) =
...
# @api texture
#------------------------------------------------------------------------------------------
proc texture*(api: P2DrawAPI, texture: Texture2D, pos: Vec3, origin: Vec2 = vec2(0.5, 0.5), scale: f32 = 1.0,
    color: Color = cWhite) =
  const indices = 6
  const vertices = 4
  ...
  r2d.draw(indices, vertices, R2D_SPRITE):
...
proc texture*(api: P2DrawAPI, texture: Texture2D, pos: Vec3, origin: Vec2 = vec2(0.5, 0.5), rotation: f32 = 0.0,
    scale: f32 = 1.0, color: Color = cWhite) =
...
proc texture*(api: P2DrawAPI, x,y,w,h: f32, color: Color, rtexture: RenderTexture) =
  const indices = 6
  const vertices = 4
  r2d.draw(indices, vertices, R2D_SPRITE):
    r2d.texture(rtexture.texture.get.id)
    r2d.color(color)
    r2d.vertex(x,  y,  0, vec2(0,0)) #A-0
    r2d.vertex(x+w,y,  0, vec2(1,0)) #B-1
    r2d.vertex(x+w,y+h,0, vec2(1,1)) #C-2
    r2d.vertex(x  ,y+h,0, vec2(0,1)) #D-3


proc texture*(api: P2DrawAPI, x,y,w,h: f32, rtexture: RenderTexture) =
  api.texture(x,y,w,h,cwhite,rtexture)
...
# |    |    |
# A----YB---B
template inline_draw_sprite(renderer: Render2D_State) {.dirty.} =
  let scaleAbsX = abs(scale.x)
  let scaleAbsY = abs(scale.y)
  let centerX = pos.x
  let centerY = pos.y
  let w = sprite.worldSize.w * scaleAbsX
  let h = sprite.worldSize.h * scaleAbsY
  let x = centerX - (w * sprite.origin.x) - sprite.worldOffsetX * sprite.origin.x * scaleAbsX
  let y = centerY - (h * sprite.origin.y) - sprite.worldOffsetY * sprite.origin.y * scaleAbsY
  let flipX = pxd.math.sign(scale.x)
  # Determine vertex positions based on flipX
  let xA = if flipX < 0: x + w else: x
  let xB = if flipX < 0: x else: x + w
  r2d.draw(indices, vertices, R2D_SPRITE):
   r2d.texture(sprite.texId)
   r2d.color(color)
   r2d.vertex(xA, y, pos.z, sprite.texCoords[0]) #A:0
   r2d.vertex(xB, y, pos.z, sprite.texCoords[1]) #B:1
   r2d.vertex(xB, y + h, pos.z, sprite.texCoords[2]) #C:2
   r2d.vertex(xA, y + h, pos.z, sprite.texCoords[3]) #D:3


proc sprite*(api: P2DrawAPI, sprite: Sprite, pos: Vec3, scaleX: f32 = 1.0, color: Color = cWhite) {.inline.} =
  const indices = 6
  const vertices = 4
  let sprite = sprite.get.addr
  let ppu_scale = pxd.render.frame.ppu
  let scale     = vec2(scaleX, 1.0)
  inline_draw_sprite(render2D.state)


proc sprite*(api: P2DrawAPI, sprite: Sprite, pos: Vec3, scale: Vec2 = vec2(1.0,1.0), color: Color = cWhite) {.inline.} =
  const indices = 6
  const vertices = 4
  let sprite = sprite.get.addr
  let ppu_scale = pxd.render.frame.ppu
...
proc sprite*(api: P2DrawAPI, sprite: Sprite, pos: Vec3, scaleX: f32 = 1.0, rotation: f32 = 0.0,
    color: Color = cWhite) {.inline.} =
  # origin is normalized
  const indices = 6
  const vertices = 4
  let sprite = sprite.get.addr
  let ppu_scale = pxd.render.frame.ppu
  if rotation == 0:
    let scale     = vec2(scaleX, scaleX)
    inline_draw_sprite(render2D.state)
  else:
    let scaleAbs = abs(scaleX)
    let sinRotation = sin(-rotation*DEG2RAD)
    let cosRotation = cos(-rotation*DEG2RAD)
    let w = sprite.worldSize.w * scaleAbs
    let h = sprite.worldSize.h * scaleAbs
    let x = pos.x - (w * sprite.origin.x) - sprite.worldOffsetX * sprite.origin.x * scaleAbs
    let y = pos.y - (h * sprite.origin.y) - sprite.worldOffsetY * sprite.origin.y * scaleAbs
    let flipX = pxd.math.sign(scaleX)
    let centerX = pos.x
    let centerY = pos.y
    template rotate(x, y: float): Vec2 =
      let dx = x - centerX
      let dy = y - centerY
      vec2(centerX + (dx * cosRotation - dy * sinRotation), centerY + (dx * sinRotation + dy * cosRotation))
    let xA = if flipX < 0: x + w else: x
    let xB = if flipX < 0: x else: x + w
    let
      v0 = rotate(xA, y)
    ...
    r2d.draw(indices, vertices, R2D_SPRITE):
      r2d.texture(sprite.texId)
      r2d.color(color)
      r2d.vertex(v0[0], v0[1], pos.z, sprite.texCoords[0]) #A:0
      r2d.vertex(v1[0], v1[1], pos.z, sprite.texCoords[1]) #B:1
      r2d.vertex(v2[0], v2[1], pos.z, sprite.texCoords[2]) #C:2
      r2d.vertex(v3[0], v3[1], pos.z, sprite.texCoords[3]) #D:3


proc sprite*(api: P2DrawAPI, sprite: Sprite, pos: Vec3, scale: Vec2 = vec2(1.0,1.0), rotation: f32 = 0.0,
    color: Color = cWhite) {.inline.} =
  # origin is normalized
  const indices = 6
  const vertices = 4
  ...
  if rotation == 0:
    ...
    let sinRotation = sin(rotation*DEG2RAD)
    let cosRotation = cos(rotation*DEG2RAD)
    let w = sprite.worldSize.w * scaleAbsX * ppu_scale
    let h = sprite.worldSize.h * scaleAbsY * ppu_scale
    let x = pos.x - sprite.origin.x * scaleAbsX * ppu_scale
    let y = pos.y - sprite.origin.y * scaleAbsY * ppu_scale
    let flipX = pxd.math.sign(scale.x)
    let centerX = pos.x
    let centerY = pos.y
    template rotate(x, y: float): Vec2 =
      let dx = x - centerX
      let dy = y - centerY
      vec2(centerX + (dx * cosRotation - dy * sinRotation) * flipX, centerY + (dx * sinRotation + dy * cosRotation))
    let
      v0 = rotate(x, y)
      v1 = rotate(x + w, y)
      v2 = rotate(x + w, y + h)
...
template cursorStep(): float {.dirty.} =
  (glyph.xadvance - fontPadding * 2) * fontSize


template lineStep(): float {.dirty.} =
  (font.lineHeight.float + render2D.textLineSpacing) * fontSize
...
template drawGlyph(glyph: var FontGlyph, pos: Vec3, scale: float = 1.0, color: Color = cWhite) =
  ...
  let w = sprite.worldSize.w * scale
  let h = sprite.worldSize.h * scale
  let x = pos.x + glyph.xoffset * scale
  var y = pos.y - glyph.yoffset * scale - h
  r2d.draw(indices, vertices, R2D_FONT):
    r2d.texture(sprite.texId)
    r2d.color(color)
...
template textDraw() {.dirty.} =
  ...
  case runeUTF:
    ...
    of " ":
      ...
      drawGlyph(glyph, cursor, fontSize, color)
      cursor.x += cursorStep()
  inc parseIndex[]
...
proc text*(api: P2DrawAPI, text: string, pos: Vec3, textScale: f32 = 1.0, color: Color = cWhite) {.inline.} =
  render2D.useShader(render2D.fontShader):
    let parseIndex = render2D.textParseIndex.addr
    let textBounds = render2D.textBounds
    let font = render2D.font
    let fontPadding = font.get.padding.float
    let fontSize = font.get.size.float / font.get.sizeMax.float * textScale
    var cursor = pos
    parseIndex[] = 0
...
proc text*(api: P2DrawAPI, text: string, x: f32, y: f32, z: f32, textScale: f32 = 1.0,
    color: Color = cWhite) {.inline.} =
...
proc setTextLineSpacing*(api: P2DrawAPI, value: float) =
  render2D.textLineSpacing = value


proc resetFont*(api: P2DrawAPI) =
  render2D.font = render2D.fontDefault


template bounds*(api: P2DrawAPI, width: float, code: untyped) =
  render2D.textBounds = vec2(width, 0)
  code
  render2D.textBounds = vec2(0, 0)
