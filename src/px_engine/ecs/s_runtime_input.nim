import std/tables
import px_engine/tools/m_utils
import px_engine/pxd/definition/internal
import definition/types


pxd.genEventAPI(EventInput, EventInputObj, input)
...
proc mouseWorldPosition*(api: IoAPI): Vec3 =
  let mx  = pxd.events.input.mouseX
  let my  = pxd.events.input.mouseY
  # normalize
  let nmx = mx / pxd.io.app.viewport.w.int
  let nmy = my / pxd.io.app.viewport.h.int
...
proc mousePosition*(api: IoAPI): Vec3 =
  result.x = pxd.events.input.mouseX.f32
  result.y = pxd.io.app.screen.h.f32 - pxd.events.input.mouseY.f32